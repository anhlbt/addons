...
# Configure logging
logging.basicConfig(level=logging.DEBUG, filename='./addon_debug.log', filemode='a', format='%(asctime)s - %(levelname)s - %(message)s')
        ...
        try:
            if self.do_translate:
                ...
                if pathlib.Path(file).is_file():
                    ...
                    srt.save(f"{os.path.splitext(file)[0]}_translated_{self.translate_to}.srt")
                    translator.quit()
                    print("Translating finished.")
                    self.report({"INFO"}, "Translating finished.")
            file = self.filepath
            if self.do_translate:
                file = f"{os.path.splitext(file)[0]}_translated_{self.translate_to}.srt"
